...
namespace Server
{
    public static class ScriptCompiler
    {
        ...
        public static IEnumerable<MetadataReference> GetCSharpReferenceAssemblies()
        {
                ...
                using (var ip = new StreamReader(path))
                {
                    ...
                    while ((line = ip.ReadLine()) != null)
                    {
                        if (line.Length > 0 && !line.StartsWith("#"))
                        {
                            try
                            {
                                list.Add(MetadataReference.CreateFromFile(line));
                            }
                            catch (FileNotFoundException)
                            {
                                list.Add(MetadataReference.CreateFromFile(Path.Combine($"C:\\Program Files (x86)\\Reference Assemblies\\Microsoft\\Framework\\.NETFramework\\v{Core.DotNet}\\", line)));
                            }
                        }
                    }
                }
            ...
            list.Add(MetadataReference.CreateFromFile(typeof(ValueTuple<>).GetTypeInfo().Assembly.Location));
            list.Add(MetadataReference.CreateFromFile(Core.ExePath));

            list.AddRange(m_AdditionalReferences.Select(a => MetadataReference.CreateFromFile(a)));

            return list;
        }

        public static CSharpCompilationOptions GetCSharpCompilerOptions(bool debug)
        {
            ...
            if (Core.HaltOnWarning)
                options.WithWarningLevel(4);

            if (Core.Is64Bit)
                options.WithPlatform(Platform.X64);
            else
                options.WithPlatform(Platform.X86);
        ...
        public static bool CompileCSScripts(bool debug, bool cache, out Assembly assembly)
        {
            ...
            foreach (var script in scriptFiles)
            {
                ...
                scriptTrees.Add(SyntaxFactory.ParseSyntaxTree(sourceText, parseOptions, Path.GetFileName(script)));
            }

            var compilation = CSharpCompilation.Create("Scripts.CS.dll", scriptTrees, GetCSharpReferenceAssemblies(), compilerOptions);
        ...
        private static void Display(EmitResult result)
        {
            if (!result.Diagnostics.IsEmpty)
            {
                ...
                foreach (var err in result.Diagnostics)
                {
                    var table = err.Severity == DiagnosticSeverity.Error ? errors : warnings;

                    var file = err.Location.SourceTree.FilePath;

                    table.TryGetValue(file, out List<string> list);

                    if (list == null)
                        table[file] = list = new List<string>();

                    list.Add($"({err.ToString().Replace(file, "")}");
                }
